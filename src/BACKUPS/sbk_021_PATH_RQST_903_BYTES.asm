...
;; add path request (images/pages) - 908 byes exe
;; 
...
THREAD_BYTES_READ     equ -12        ; worker local bytes-read storage
THREAD_FILE_BUF       equ -(FILEBUF_SIZE + 12) ; worker local file buffer
...
client_thread:
    ...
    lea  ecx, [ebp+THREAD_FILE_BUF]  ; thread-local request buffer
    push ecx                         ; request buffer
    push esi                         ; client socket
    call recv                        ; receive HTTP request
    cmp  eax, 0                      ; did client send anything?
    jle  thread_close_client         ; close if recv failed or disconnected
    lea  ecx, [ebp+THREAD_FILE_BUF]  ; thread-local request buffer
    mov  byte ptr [ecx+eax], 0       ; terminate received request
    
    ;;; parse requested path
    cmp  dword ptr [ebp+THREAD_FILE_BUF], 20544547h ; request starts with "GET "?
    jne  thread_send_404                            ; reject non-GET request
    cmp  byte ptr [ebp+THREAD_FILE_BUF+4], '/'      ; path starts with slash?
    jne  thread_send_404                            ; reject malformed path

    lea  edx, [ebp+THREAD_FILE_BUF+5]               ; point after "GET /"
...
thread_path_ready:
    ...
    lea  edx, [ebp+THREAD_FILE_BUF+5] ; filename starts after "GET /"
    jmp  thread_open_file            ; open requested file


thread_use_start_page:

    ;;; use configured start page
    mov  edx, dword ptr [startPagePtr] ; configured start page filename
...
thread_file_loop:
    ...
    push FILEBUF_SIZE                ; bytes to read
    lea  eax, [ebp+THREAD_FILE_BUF]  ; thread-local file buffer
    push eax                         ; file chunk buffer
    push edi                         ; file handle
    call ReadFile                    ; read file data
    test eax, eax                    ; did ReadFile fail?
